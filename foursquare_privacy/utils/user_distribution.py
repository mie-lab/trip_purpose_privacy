...
def get_user_dist_mae(results):
    ...
    pred_columns = [f"pred_{val.split('_')[1]}" for col in gt_dummies.columns if col != "user_id"]
    if len(gt_dummies.columns) < len(pred_dummies.columns):
        missing_cols = [col for col in pred_columns if col not in pred_dummies.columns]
        for col_name in missing_cols:
            pred_dummies[f"pred_{col_name}"] = 0
    # reorder
